    ...
    for (let lv = im.maxLevel; lv >= lo; lv--) {
      ...
      for (let ty = r.ty0; ty <= r.ty1; ty++) {
        for (let tx = r.tx0; tx <= r.tx1; tx++) {
          ...
          // Non-negative modulo so negative (provider) levels index the
          // palette instead of resolving to undefined.
          const col = LEVEL_COLORS[((lv % LEVEL_COLORS.length) + LEVEL_COLORS.length) % LEVEL_COLORS.length];
